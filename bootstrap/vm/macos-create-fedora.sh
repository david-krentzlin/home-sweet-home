...
echo "Apply dev config:   ./bootstrap/vm/apply-user.sh --vm-name $INSTANCE_NAME --target dev --context work"
echo "Apply agent config: ./bootstrap/vm/apply-user.sh --vm-name $INSTANCE_NAME --target agent --context work"
echo "The helper clones /workspaces/home-sweet-home in the VM if it is missing."
